...
class ProductTest < ActiveSupport::TestCase
  ...
  test 'should filer product by name and sort them' do
  ...
  test 'should sort by creation date' do
    products(:two).touch
    assert_equal [products(:another_tv), products(:one), products(:two)], Product.recent.to_a
  end

  test 'should not find "videogame" and "100" as min price' do
  ...
  test 'should get all products when no paramaters' do
